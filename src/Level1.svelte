<script lang="ts">
	...
	import { Planet, makePlanetlvl1 } from './lib/planet';
	...
	import { Target, createTargetlvl1 } from './lib/target';
	...
	import { defaultPlugins } from './lib/ecs/plugins';
	...
	const ecs = new ECS()
		...
		.insertResource(
			new KeysToTrack(['w', 'a', 's', 'd', ' ', 'ArrowUp', 'ArrowDown', 'ArrowRight', 'ArrowLeft', '+', '-'])
		)
	...
	const ugs = gspeed.subscribe((s) => {
		const settings = ecs.getResource(TimeData);
	...
	const uw = win.subscribe((w) => {
		if (w) localStorage.setItem('c1', 'true');
	});
		...
		{:else}
			<div class="velocity" transition:fly={{ y: 200, duration: 500, easing: cubicOut }}>
				...
				<input
					...
					bind:value={$speed}
					on:input={() => setSpeed.send(new SetSpeedEvent(parseInt(speedInput.value)))}
				/>
			</div>
		{/if}
	{:else}
		<div class="victory" transition:fly={{ y: -1000, delay: 500, duration: 2000, easing: expoInOut }}>
			...
			<div>
				...
				<button on:click={() => screen.set('level2')}><span>Next</span></button>
			</div>
		</div>
	{/if}
</main>
